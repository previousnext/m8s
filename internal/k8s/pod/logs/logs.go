...
func Stream(ctx context.Context, w io.Writer, client *kubernetes.Clientset, namespace, name string) error {
	...
	for i, container := range pod.Status.InitContainerStatuses {
		pod, err := watcher.waitForPod(ctx, func(p *v1.Pod) bool {
			...
			if waiting.Message != "" {
				fmt.Fprintf(w, "[%s] %s", container.Name, waiting.Message)
			}
		...
		if err := printContainerLogs(ctx, w, pods, name, container.Name, followContainer); err != nil {
		...
		if terminated.ExitCode != 0 {
			...
			fmt.Fprintf(w, "[%s] %s", container.Name, message)
...
func printContainerLogs(ctx context.Context, w io.Writer, pods corev1.PodExpansion, podName, containerName string, follow bool) error {
	...
	return streamLogs(ctx, w, containerName, rc)
}

func streamLogs(ctx context.Context, w io.Writer, containerName string, rc io.Reader) error {
